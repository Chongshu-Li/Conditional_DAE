...
from visualisation import plot_pr_curve
...
def eval_anomalies_batched(trainer, dataset, get_scores, batch_size=32, threshold=None, get_y=lambda batch: batch[1],
    ...
    ap = average_precision_score(y_true_.cpu().numpy().astype(bool),
                                 y_pred_.cpu().numpy().astype(float))
    
    aucroc = roc_auc_score(y_true_.cpu().numpy().astype(bool),
                           y_pred_.cpu().numpy().astype(float))
    ...
    if return_dice:
        ...
        if filter_cc:
            ...
            return ap, aucroc, max_dice, threshold, post_cc_max_dice, y_true_, y_pred_
        return ap, max_dice, threshold, y_true_, y_pred_
    return ap, aucroc, y_true_, y_pred_


def evaluate(id: str = "model", split: str = "test", use_cc: bool = True, dataset = None, pr_curve = False):
    trainer = denoising(id, data=None, lr=1e-4, depth=3,
    ...
    ap, aucroc, max_dice, threshold, post_cc_max_dice, y_true_, y_pred_ = results

    print(f"\n--- Final Evaluation ---")
    print(f"Average Precision (AUPRC): {results[0]:.4f}")
    print(f"AUCROC: {aucroc:.4f}")
    print(f"Max Dice: {results[2]:.4f}")
    if use_cc:
        print(f"Max Post-CC Dice: {results[4]:.4f}")
    print(f"Optimal Threshold: {results[3]:.4f}")

    if pr_curve == True:
        plot_pr_curve(y_true_, y_pred_)
