...
def preprocess(datapath: Path):
    ...
    all_imgs = sorted([p for p in datapath.iterdir() if not p.name.startswith('.')])


    splits_path = Path(__file__).parent / "data" / "brats2021_preprocessed" / "data_splits"
    ...
    for split in ["train", "val", "test"]:
        ...
        paths = [datapath / x.strip() for x in open(splits_path / split / "scans.csv").readlines() if not x.strip().startswith('.')]
