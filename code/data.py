...
def skip_healthy_fn(item):
    return item[1].sum() <= 0 # Resutnr TRUE for healthy slice (no tumor)

def skip_tumour_fn(item):
    return item[1].sum() > 0 # Resutnr TRUE for tumour slice
...
class PatientDataset(torch.utils.data.Dataset): # Identify and filter individual slices based on whether they are healthy or contain tumor
    """
...
class BrainDataset(torch.utils.data.Dataset): 
    ...
    def __init__(self, dataset="brats2021", split="val", n_tumour_patients=None, n_healthy_patients=None,
        ...
        if dataset == "brats2021":
            datapath = Path(__file__).parent / "data" / "brats2021_preprocessed"
            path = datapath / f"npy_{split}"
        else:
            raise ValueError(f"Dataset '{dataset}' is not supported.")
        
        self.skip_healthy = skip_healthy_fn
        self.skip_tumour = skip_tumour_fn
